...
async function savePlayerEdit(playerId,isNew){
  ...
  // Aliases: split on commas, lower-case, trim, drop blanks and duplicates.
  var aliasEl=document.getElementById('pe-aliases');
  var aliases=aliasEl?aliasEl.value.split(',').map(function(a){return a.trim().toLowerCase();}).filter(function(a,i,arr){return a&&arr.indexOf(a)===i;}):[];
  PLAYERS=getPlayers();
  ...
  if(isNew){
    ...
    var newP={id:newId,ign:ign,nick:nick,role:role,status:status,aliases:aliases};
    try{
    ...
    var updatedP=Object.assign({},PLAYERS[idx],{ign,nick,role,status,aliases:aliases});
    try{
